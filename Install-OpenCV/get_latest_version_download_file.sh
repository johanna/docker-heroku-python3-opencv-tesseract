...
version="$(wget -q -O - http://sourceforge.net/projects/opencvlibrary/files/opencv-unix | egrep -m1 -o 'Download opencv-([0-9](\.[0-9]+)+(-[-a-zA-Z0-9]+)*).zip?' | sed -e 's/Download opencv-\(.*\).zip/\1/')"
